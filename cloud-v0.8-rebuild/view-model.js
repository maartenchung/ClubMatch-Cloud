...
function createPlayerView(player,meta){
  ...
  return Object.freeze({
    id:player.playerId,name:meta.name||meta.full_name||player.playerId,
    ...
    initialRole:player.initialRole,role,position:role==='FIELD'?(player.currentPosition||''):'',startedPosition:player.startedPosition||'',
    playSeconds:second(player.playSeconds),benchSeconds:second(player.benchSeconds),currentStintSeconds:second(player.currentStintSeconds),
    inCount:second(player.inCount),outCount:second(player.outCount),substitutionCount:second(player.substitutionCount),
    lastSubstitutionSecond:player.lastSubstitutionSecond==null?null:second(player.lastSubstitutionSecond),
    lastSubstitutionDirection:player.lastSubstitutionDirection||null,changeState,statusStyle:STATUS_STYLES[changeState],metrics
  });
}

function createTimeline(events){
  return Object.freeze([...(events||[])].map((event,index)=>Object.freeze({
    ...event,id:event.id||event.clientEventId||event.client_event_id||`event-${index+1}`,
...
function createLiveViewModel(input){
  ...
  const byId={};Object.values(liveState.players).forEach(player=>{byId[player.playerId]=createPlayerView(player,playerMetadata(input.playersById,player.playerId))});Object.freeze(byId);
...
global.ClubMatchV08ViewModel={STATUS_STYLES,METRIC_DEFINITIONS,formatClock,formatMinute,createLiveViewModel,validateLiveViewModel};
