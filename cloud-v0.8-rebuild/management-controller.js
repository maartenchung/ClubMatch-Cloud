...
function createManagementController(options={}){
  ...
  const pending={teamSeasons:null,context:null,userAdmin:null};
  function snapshot(){
  ...
  function singleFlight(key,loader){if(pending[key])return pending[key];pending[key]=Promise.resolve().then(loader).finally(()=>{pending[key]=null});return pending[key]}
  function loadTeamSeasons(){return singleFlight('teamSeasons',async()=>{const data=await rpc('get_my_team_seasons');teamSeasons=Array.isArray(data)?data:[];return emit()})}
  function loadContext(){return singleFlight('context',async()=>{context=await rpc('get_management_context_v08')||{clubs:[],can_create_club:false};return emit()})}
  function loadUserAdmin(){return singleFlight('userAdmin',async()=>{userAdmin=await rpc('get_user_admin_v08')||{is_platform_admin:false,can_manage_users:false,clubs:[],memberships:[]};return emit()})}
  ...
  return Object.freeze({loadTeamSeasons,loadContext,loadUserAdmin,loadAll,load,createTeamSeason,updateNames,savePlayer,addPlayer,setMembership,inviteUser,get state(){return snapshot()},get pending(){return Object.freeze({...pending})},normalizePositions:positions})
}
