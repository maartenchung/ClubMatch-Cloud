...
function createManagementController(options={}){
  ...
  function state(){return Object.freeze({teamSeasons:Object.freeze([...teamSeasons]),context:Object.freeze({...context,clubs:Object.freeze([...(context.clubs||[])].map(c=>Object.freeze({...c})))}),teamSeasonId,admin:admin?Object.freeze({...admin,players:Object.freeze([...(admin.players||[])].map(p=>Object.freeze({...p,preferred_positions:Object.freeze([...(p.preferred_positions||[])] )}))}):null})}
  ...
  async function createTeamSeason(input={}){const clubId=input.newClub?null:(input.clubId||null);if(input.newClub)invariant(context.can_create_club,'Alleen platformbeheer kan een nieuwe club maken');invariant(clean(input.teamName),'Teamnaam is verplicht');invariant(clean(input.seasonName),'Seizoennaam is verplicht');const result=await rpc('create_team_season_v08',{p_club_id:clubId,p_club_name:input.newClub?clean(input.clubName):null,p_team_name:clean(input.teamName),p_season_name:clean(input.seasonName),p_display_name:clean(input.displayName)||null});invariant(result?.team_season_id,'Nieuw team/seizoen is onvolledig');teamSeasonId=result.team_season_id;admin=result;await loadTeamSeasons();await loadContext();return emit()}
  ...
  return Object.freeze({loadTeamSeasons,loadContext,load,createTeamSeason,updateNames,savePlayer,addPlayer,get state(){return state()},normalizePositions:positions})
}
