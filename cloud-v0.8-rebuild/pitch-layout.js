/* ClubMatch Cloud v0.8 - gedeeld veldmodel voor voorbereiding en live */
(function(global){
'use strict';
const POSITION_LABELS=Object.freeze({
  ...
  RDM:'Rechter controlerende middenvelder',LDM:'Linker controlerende middenvelder',DM:'Controlerende middenvelder',RM:'Rechtermiddenvelder',RCM:'Rechter centrale middenvelder',CM:'Centrale middenvelder',LCM:'Linker centrale middenvelder',LM:'Linkermiddenvelder',AM:'Aanvallende middenvelder',
...
function geometry(code='4-3-3'){
  const rows=formationRows(code),n=rows.length;
  const top=n===5?11:15,bottom=89,step=n>1?(bottom-top)/(n-1):0;
  const slots=[];
  rows.forEach((row,rowIndex)=>{
    const count=row.length;
    row.forEach((position,index)=>{
      const x=count===1?50:10+(80*index/(count-1));
      const y=top+step*rowIndex;
      slots.push(Object.freeze({position,label:positionLabel(position),fullLabel:slotLabel(position),x:Number(x.toFixed(2)),y:Number(y.toFixed(2)),row:rowIndex,index,count}));
    });
  });
...
global.ClubMatchV08PitchLayout={POSITION_LABELS,FORMATION_ROWS,formationRows,positions,positionLabel,slotLabel,geometry,slot};
