...
vm.runInContext(fs.readFileSync(new URL('./pitch-layout.js',import.meta.url),'utf8'),context);
...
const oldRw=P.state.players.find(p=>p.position==='RW');assert.ok(oldRw);
P.placeStarter('p12','RW');assert.equal(P.state.players.find(p=>p.playerId==='p12').starter,true);assert.equal(P.state.players.find(p=>p.playerId==='p12').position,'RW');assert.equal(P.state.players.find(p=>p.playerId===oldRw.playerId).starter,false,'drop on occupied slot replaces starter');assert.equal(P.state.players.filter(p=>p.starter).length,11);
const st=P.state.players.find(p=>p.position==='ST');P.placeStarter('p12','ST');assert.equal(P.state.players.find(p=>p.playerId==='p12').position,'ST');assert.equal(P.state.players.find(p=>p.playerId===st.playerId).position,'RW','starter-to-starter drop swaps positions');
P.moveToBench('p12');assert.equal(P.state.players.find(p=>p.playerId==='p12').starter,false);assert.equal(P.state.players.filter(p=>p.starter).length,10);P.placeStarter('p12','ST');assert.equal(P.state.players.filter(p=>p.starter).length,11);
P.setFormation('4-2-3-1');assert.equal(P.state.meta.formationCode,'4-2-3-1');assert.ok(P.state.players.filter(p=>p.starter).every(p=>!p.position||context.ClubMatchV08Formation.getFormation('4-2-3-1').slots.includes(p.position)),'formation switch clears incompatible positions');
P.applyFormation('4-2-3-1');assert.equal(P.state.players.filter(p=>p.starter&&p.position).length,11);assert.equal(P.validate().ok,true,P.validate().errors.join(' · '));
...
P.setFormation('4-2-3-1');await assert.rejects(()=>P.start(),/laatste wijzigingen eerst op/,'dirty saved preparation cannot start');
console.log('PASS preparation-controller: roster + formation-scoped positions + drag placement + replacement/swap + save-before-start + restore');
