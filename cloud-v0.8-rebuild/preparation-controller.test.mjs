...
const roster=Array.from({length:13},(_,i)=>({player_id:`p${i+1}`,legacy_key:`k${i+1}`,full_name:`Speler ${i+1}`,display_name:`P${i+1}`,shirt_number:i+1,preferred_positions:[]}));
...
const client={async rpc(name,params={}){
  ...
  if(name==='get_match_snapshot')return {data:{match:{id:'m1',team_season_id:'ts1',opponent_name:'DEM',official_duration_minutes:80,formation_code:'4-3-3',status:'draft'},players:roster.map((p,i)=>({...p,attendance_status:'present',selected:i<12,is_starter:i<11,starting_position:i<11?['GK','RB','RCB','LCB','LB','CM1','CM2','AM','RW','ST','LW'][i]:null}))},error:null};
...
await P.loadTeam('ts1');assert.equal(P.state.players.length,13);assert.equal(P.state.validation.starterCount,0);
P.setMeta({opponentName:'DEM',matchDate:'2026-08-29',scheduledTime:'11:15',officialDurationMinutes:80,formationCode:'4-3-3'});
const pos=['GK','RB','RCB','LCB','LB','CM1','CM2','AM','RW','ST','LW'];
for(let i=0;i<12;i++)P.patchPlayer(`p${i+1}`,{selected:true});
for(let i=0;i<11;i++)P.patchPlayer(`p${i+1}`,{starter:true,position:pos[i]});
assert.equal(P.validate().ok,true,P.validate().errors.join(' · '));
const saved=await P.save();assert.equal(saved.match.id,'m-new');assert.equal(savedParams.p_players.length,13);assert.equal(savedParams.p_players.filter(p=>p.starter).length,11);assert.equal(P.state.meta.matchId,'m-new');
await P.start();assert.equal(startParams.p_match_id,'m-new');assert.equal(startParams.p_client_event_id,'evt-start');
P.patchPlayer('p1',{attendance:false});assert.equal(P.state.players[0].starter,false);assert.equal(P.validate().ok,false);
await P.openExisting({match_id:'m1',match_date:'2026-08-30',scheduled_time:'10:30:00'});assert.equal(P.state.meta.matchId,'m1');assert.equal(P.state.players.filter(p=>p.starter).length,11);assert.equal(P.state.players.filter(p=>p.selected).length,12);assert.equal(P.state.meta.scheduledTime,'10:30');assert.equal(P.validate().ok,true);
console.log('PASS preparation-controller: roster + 11 starters + save + start + restore');
