...
async function refreshOpenMatches({restore=false}={}){
  ...
  box.innerHTML='';matches.forEach(match=>{const card=document.createElement('button');card.type='button';card.className='matchChoice';card.dataset.matchId=match.match_id;card.innerHTML=`<b>${esc(match.team_name)} — ${esc(match.opponent_name)}</b><span>${esc(match.club_name||'')} · ${esc(match.match_date||'')} · ${esc(match.scheduled_time||'')} · ${esc(match.status)} · status v${esc(match.state_version??'—')}</span>`;card.onclick=()=>run(match.status==='draft'||match.status==='scheduled'?'Voorbereiding openen':'Wedstrijd openen',()=>openMatchSummary(match));box.appendChild(card)});
...
async function enterAuthenticatedApp({restore=true}={}){
  const result=await security.refreshMfa();securityUi.render(result);
  const session=(await client.auth.getSession()).data.session;setSessionIdentity(session);
...
async function bootstrapSession(){
  ...
  if(!data.session){enterSignedOut();return}
  await enterAuthenticatedApp({restore:true});
}
...
function bindActions(){
  ...
  byId('refreshMatchesBtn').onclick=()=>run('Wedstrijden verversen',()=>refreshOpenMatches({restore:!runtime.activeMatchId&&!preparationUi.visible}));byId('refreshLiveBtn').onclick=()=>run('Live-status verversen',()=>runtime.refresh('handmatig'));
  ...
  document.querySelectorAll('[data-clock]').forEach(btn=>btn.onclick=()=>run('Wedstrijdklok bijwerken',async()=>{const input={clockAction:btn.dataset.clock};if(input.clockAction==='injury_time')input.minutes=Number(byId('injuryMinutes').value);const result=await runtime.advanceClock(input);await refreshOpenMatches();return result}));
...
async function init(){
  try{
    ...
    await ensureModule('action-policy.js',()=>!!global.ClubMatchV08ActionPolicy?.createActionPolicy,'actiebeleid');
    await ensureModule('lifecycle-sync.js',()=>!!global.ClubMatchV08LifecycleSync?.createLifecycleSync,'herstelsynchronisatie');
    await ensureModule('match-selection.js',()=>!!global.ClubMatchV08MatchSelection?.createMatchSelection,'wedstrijdselectie');
    ...
    await ensureModule('security-ui.js',()=>!!global.ClubMatchV08SecurityUi?.createSecurityUi,'beveiligingsscherm');
    ...
    security=global.ClubMatchV08Security.createSecurityController({client,onChange:state=>securityUi?.render?.(state)});
    securityUi=global.ClubMatchV08SecurityUi.createSecurityUi({document,
      onRequestReset:email=>run('Resetmail aanvragen',async()=>{const result=await security.requestPasswordReset(email,RESET_REDIRECT);securityUi.setResetNote(result.message);return result}),
      ...
      onVerifyLoginMfa:code=>run('2FA verifiëren',async()=>{const state=await security.verifyLoginMfa(code);securityUi.render(state);byId('loginMfaCode').value='';securityUi.showApp();await loadSignedInContext({restore:true});return state}),
      onLogout:()=>run('Uitloggen',async()=>{const {error}=await client.auth.signOut();if(error)throw error;enterSignedOut()}),
      onBeginEnrollment:()=>run('Authenticator toevoegen',async()=>{const state=await security.beginTotpEnrollment();securityUi.render(state);return state}),
      ...
      onVerifyEnrollment:code=>run('2FA activeren',async()=>{const state=await security.verifyTotpEnrollment(code);securityUi.render(state);byId('mfaEnrollCode').value='';return state}),
      onCancelEnrollment:()=>{security.cancelEnrollment();securityUi.render(security.state)},
      onUnenroll:factorId=>{if(confirm('Deze 2FA-factor verwijderen? Zorg dat je een andere factor beschikbaar hebt.'))run('2FA-factor verwijderen',async()=>{const state=await security.unenroll(factorId);securityUi.render(state);return state})}
    });
    ...
    client.auth.onAuthStateChange((event,session)=>{if(event==='SIGNED_OUT')enterSignedOut();else if(event==='PASSWORD_RECOVERY'){stopUserWork();securityUi.showRecovery(true)}else if(session)setSessionIdentity(session)});
    await bootstrapSession();status('ClubMatch Cloud v0.8 gereed.',true);
