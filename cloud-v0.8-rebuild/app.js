...
async function init(){
  try{
    ...
    await ensureModule('security-controller.js',()=>!!global.ClubMatchV08Security?.createSecurityController,'security-controller.js');
    await ensureModule('security-ui.js',()=>!!global.ClubMatchV08SecurityUi?.createSecurityUi,'security-ui.js');
    ...
    securityUi=global.ClubMatchV08SecurityUi.createSecurityUi({document,
      ...
      onRefreshSecurity:()=>run('Security verversen',async()=>{const state=await security.refreshMfa();securityUi.render(state);return state}),
