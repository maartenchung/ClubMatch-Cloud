...
let client=null,runtime=null,renderer=null,lifecycleSync=null,matchSelection=null,lastFrame=null,busy=false;
...
async function openMatch(matchId,{remember=true}={}){await runtime.start(matchId);if(remember)matchSelection?.remember(matchId);markActiveMatch(matchId);return runtime.viewModel}
...
async function refreshOpenMatches({restore=false}={}){
  ...
  box.innerHTML='';matches.forEach(match=>{const card=document.createElement('button');card.type='button';card.className='matchChoice';card.dataset.matchId=match.match_id;card.innerHTML=`<b>${esc(match.team_name)} — ${esc(match.opponent_name)}</b><span>${esc(match.club_name||'')} · ${esc(match.match_date||'')} · ${esc(match.scheduled_time||'')} · ${esc(match.status)} · state v${esc(match.state_version??'—')}</span>`;card.onclick=()=>run('Wedstrijd openen',()=>openMatch(match.match_id));box.appendChild(card)});
  if(runtime?.activeMatchId&&matches.some(match=>match.match_id===runtime.activeMatchId))markActiveMatch(runtime.activeMatchId);
  else if(restore){const chosen=matchSelection?.choose(matches);if(chosen)await openMatch(chosen)}
  return matches;
}
function showSession(session){const signed=!!session?.user;byId('authPanel').classList.toggle('hidden',signed);byId('appPanel').classList.toggle('hidden',!signed);byId('sessionEmail').textContent=signed?session.user.email||session.user.id:'';if(!signed){runtime?.stop?.();resetLiveUi()}}
async function bootstrapSession(){const {data,error}=await client.auth.getSession();if(error)throw error;showSession(data.session);if(data.session)await refreshOpenMatches({restore:true})}

function bindActions(){
  byId('loginBtn').onclick=()=>run('Inloggen',async()=>{const email=byId('email').value.trim(),password=byId('password').value;const {data,error}=await client.auth.signInWithPassword({email,password});if(error)throw error;showSession(data.session);await refreshOpenMatches({restore:true})});
  ...
  byId('refreshMatchesBtn').onclick=()=>run('Wedstrijden verversen',()=>refreshOpenMatches({restore:!runtime.activeMatchId}));byId('refreshLiveBtn').onclick=()=>run('Live status verversen',()=>runtime.refresh('manual'));
