...
let resetArgs=null,updated=null,signedOut=0,verified=false,enrolled=false,removed=false,aalCalls=0;
...
const client={auth:{
  ...
  mfa:{
    ...
    async getAuthenticatorAssuranceLevel(){aalCalls++;return {data:{currentLevel:verified?'aal2':'aal1',nextLevel:'aal2'},error:null}},
...
let state=await S.refreshMfa();assert.equal(state.mfa.needsChallenge,true);assert.equal(aalCalls,1);
...
let zeroAalCalls=0;
const noFactorClient={auth:{importRecoveryFromUrl(){return {data:{recovery:false},error:null}},mfa:{async listFactors(){return {data:{all:[],totp:[],phone:[]},error:null}},async getAuthenticatorAssuranceLevel(){zeroAalCalls++;return {data:{currentLevel:'aal1',nextLevel:'aal2'},error:null}}}}};
const Z=context.ClubMatchV08Security.createSecurityController({client:noFactorClient});const zero=await Z.refreshMfa();assert.equal(zero.mfa.currentLevel,'aal1');assert.equal(zero.mfa.nextLevel,'aal1');assert.equal(zero.mfa.needsChallenge,false);assert.equal(zeroAalCalls,0,'zonder MFA-factor mag geen tweede AAL/user netwerkcontrole nodig zijn');
console.log('PASS security-controller: recovery + TOTP lifecycle + zero-factor fast bootstrap');
