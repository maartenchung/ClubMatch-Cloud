...
function createTrainingController(options={}){
 const client=options.client;invariant(client?.rpc,'Cloud-client met RPC is verplicht');let teamSeasons=[],teamSeasonId='',library=[],dashboard={sessions:[],players:[],model:null};
 function state(){return freeze({teamSeasons:[...teamSeasons],teamSeasonId,library:[...library],dashboard:{...dashboard,sessions:[...(dashboard.sessions||[])],players:[...(dashboard.players||[])]}})}
 ...
 async function loadLibrary(){const data=await rpc('get_my_training_library_v08');library=Array.isArray(data)?data:[];return emit()}
 ...
 async function createSession(input={}){invariant(teamSeasonId,'Kies eerst een team/seizoen');invariant(input.trainingDate,'Trainingsdatum is verplicht');const created=await rpc('create_training_session_v08',{p_team_season_id:teamSeasonId,p_training_date:input.trainingDate,p_scheduled_time:input.scheduledTime||null,p_title:String(input.title||'Training').trim()||'Training',p_notes:String(input.notes||'').trim()||null});await load(teamSeasonId);await loadLibrary();return {created,state:state()}}
