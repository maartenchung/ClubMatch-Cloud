...
function createTrainingController(options={}){
 ...
 async function createSession(input={}){invariant(teamSeasonId,'Kies eerst een team/seizoen');invariant(input.trainingDate,'Trainingsdatum is verplicht');await rpc('create_training_session_v08',{p_team_season_id:teamSeasonId,p_training_date:input.trainingDate,p_scheduled_time:input.scheduledTime||null,p_title:String(input.title||'Training').trim()||'Training',p_notes:String(input.notes||'').trim()||null});return load(teamSeasonId)}
 ...
 return Object.freeze({loadTeams,load,createSession,saveReview,deleteSession,get state(){return state()}})
}
