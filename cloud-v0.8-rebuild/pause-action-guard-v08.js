...
function status(){return String(snapshot?.match?.status||'')}
function clockRunning(){return status()==='live'&&String(snapshot?.state?.clock_status||'')==='running'}
...
function onConfirmed(event){snapshot=event?.detail?.snapshot||event?.detail?.runtime?.snapshot||snapshot;sync()}
function boot(){ensureStyle();stampBuild();bind();global.addEventListener?.('clubmatch:v08-confirmed',onConfirmed);global.addEventListener?.('clubmatch:v08-runtime-ready',event=>{snapshot=event?.detail?.runtime?.snapshot||snapshot;sync()});global.addEventListener?.('pageshow',sync);global.setTimeout(sync,120)}
