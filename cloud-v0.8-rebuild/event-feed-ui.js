...
function createEventFeedUi(doc=global.document){
 ...
 function mount(){const timeline=doc.getElementById('v08Timeline'),card=timeline?.closest('.card');if(!card)return false;if(!controls){controls=doc.createElement('div');controls.className='controls';controls.style.marginBottom='8px';controls.innerHTML='<button type="button" class="secondary" data-feed="match">Wedstrijd</button><button type="button" class="secondary" data-feed="analysis">Analyse</button><button type="button" class="secondary" data-feed="all">Alles</button>';const heading=card.querySelector('h2');heading?.after(controls);more=doc.createElement('button');more.type='button';more.className='secondary hidden';more.style.cssText='margin-top:8px;width:100%';more.onclick=()=>{visibleLimit+=20;apply()};timeline.after(more);controls.querySelectorAll('[data-feed]').forEach(b=>b.onclick=()=>{mode=b.dataset.feed;visibleLimit=12;apply()})}if(!observer){observer=new MutationObserver(apply);observer.observe(timeline,{childList:true,subtree:true})}apply();return true}
