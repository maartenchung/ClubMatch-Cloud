/* ClubMatch Cloud v0.8 - dashboard/history/action/rating/player360 controller */
(function(global){
'use strict';
...
function createDashboardController(options={}){
  const client=options.client;invariant(client?.rpc,'Cloud client with rpc is required');let teamSeasons=[],dashboard=normalize(),playerOverview=null;
  function snapshot(){return freeze({teamSeasons:[...teamSeasons],dashboard,playerOverview})}
  ...
  async function load(teamSeasonId=null){dashboard=normalize(await rpc('get_cloud_dashboard_v2',{p_team_season_id:teamSeasonId||null}));playerOverview=null;return emit()}
