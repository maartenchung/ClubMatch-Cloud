/* ClubMatch Cloud v0.8 - stable tablet matchday layout 20260829.0415 */
(function(global){
'use strict';
const doc=global.document;
const BUILD='20260829.0415';
...
#v08LiveActionField .lafOwn,#v08LiveActionField .lafOpp{width:120px!important;min-height:52px!important;padding:6px 8px!important;border-radius:11px!important}#v08LiveActionField .lafName{font-size:11px!important;line-height:1.12!important}#v08LiveActionField .lafPos{font-size:8px!important;line-height:1.1!important}#v08LiveActionField .lafNo{font-size:11px!important}#v08LiveActionField .lafDir{font-size:10px!important;font-weight:900!important}
...
@media(pointer:coarse){#v08LiveActionField .lafActionDock button{min-height:52px!important}#v08LiveActionField .lafOwn,#v08LiveActionField .lafOpp{width:124px!important;min-height:54px!important}.v08LiveFormationControl button{min-height:48px}}
...
function forceActionField(){
 ...
 const api=global.ClubMatchV08AnalystLiveInput;
 let panel=doc.getElementById('v08LiveActionField');
...
function relayoutActionField(){
 ...
 const ownY=y=>54+((y-15)/(89-15))*39,oppY=y=>46-((y-15)/(89-15))*39;
 pitch.querySelectorAll('.lafOwn').forEach(node=>{const slot=ownMap.get(posCode(node));if(!slot)return;node.style.left=`${clamp(slot.x,8,92)}%`;node.style.top=`${clamp(ownY(slot.y),54,94)}%`});
 pitch.querySelectorAll('.lafOpp').forEach(node=>{const slot=oppMap.get(posCode(node));if(!slot)return;node.style.left=`${clamp(100-slot.x,8,92)}%`;node.style.top=`${clamp(oppY(slot.y),6,46)}%`});
...
function nearestAssignments(code){
 ...
 const oldMap=new Map(oldGeom.slots.map(s=>[s.position,s]));let targets=nextGeom.slots.slice(),assigned=[];
 const exact=[];for(const p of players){const position=p.current_position||p.starting_position||'';const hit=targets.find(t=>t.position===position);if(hit){exact.push({player_id:p.player_id,position:hit.position});targets=targets.filter(t=>t.position!==hit.position)}}
 assigned.push(...exact);const used=new Set(exact.map(a=>a.player_id));
 const rest=players.filter(p=>!used.has(p.player_id)).sort((a,b)=>{const ap=oldMap.get(a.current_position||a.starting_position||'')?.y??50,bp=oldMap.get(b.current_position||b.starting_position||'')?.y??50;return bp-ap});
...
function ensureFormationControl(){
 ...
 const select=box.querySelector('#v08LiveFormationSelect'),button=box.querySelector('#v08LiveFormationApply'),status=box.querySelector('#v08LiveFormationStatus');select.value=lastSnapshot?.match?.formation_code||'4-3-3';
 button.onclick=async()=>{if(!runtime?.changeFormation)return;button.disabled=true;status.className='v08LiveFormationStatus';status.textContent='Formatie opslaan…';try{const code=select.value,assignments=nearestAssignments(code);await runtime.changeFormation({formationCode:code,assignments,updateBasis:false});status.className='v08LiveFormationStatus ok';status.textContent=`${code} toegepast ✓`}catch(error){console.error(error);status.className='v08LiveFormationStatus bad';status.textContent=error.message||String(error)}finally{button.disabled=false}}
}
...
function moveQuickRegistration(){
 const bar=doc.getElementById('v08PossessionBar'),workspace=doc.getElementById('v08LiveMatchWorkspace');if(!bar||!workspace)return false;
 let card=doc.getElementById('v08QuickRegistrationCard');if(!card){card=doc.createElement('section');card.id='v08QuickRegistrationCard';card.className='card';card.innerHTML='<h2>Snelle registratie</h2><div class="v08QuickRegistrationHint">Acties en balbezit snel registreren. Dit staat bewust onder Live monitoring en Gebeurtenissen zodat de live veldopstellingen overzichtelijk blijven.</div>';workspace.appendChild(card)}
 if(bar.parentElement!==card)card.appendChild(bar);return true
}
...
function cascade(){schedule();global.setTimeout?.(schedule,80);global.setTimeout?.(schedule,250);global.setTimeout?.(schedule,700)}
function onConfirmed(event){runtime=event?.detail?.runtime||runtime;lastSnapshot=event?.detail?.snapshot||runtime?.snapshot||lastSnapshot;if(isLive())cascade()}
function boot(){ensureStyles();stampBuild();global.addEventListener?.('clubmatch:v08-runtime-ready',event=>{runtime=event?.detail?.runtime||runtime});global.addEventListener?.('clubmatch:v08-confirmed',onConfirmed);global.addEventListener?.('pageshow',()=>{if(isLive())cascade()});global.addEventListener?.('resize',()=>{if(isLive())global.setTimeout?.(schedule,80)})}
if(doc?.readyState==='loading')doc.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
global.ClubMatchV08TabletHotfix=Object.freeze({BUILD,sync,forceActionField,relayoutActionField,nearestAssignments,moveQuickRegistration});
